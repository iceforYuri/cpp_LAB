...
Client::Client() : clientSocket(INVALID_SOCKET), isLoggedIn(false)
{
    // 构造函数，初始化成员变量
    // 注意：clientSocket 已在初始化列表中设置为 INVALID_SOCKET
    // isLoggedIn 已在初始化列表中设置为 false
    // currentUsername 和 currentUserType 默认为空字符串
}
...
bool Client::sendRequest(const std::string &request, std::string &response)
{
    // 发送请求
...
void Client::handleBrowseProducts()
{
...
void Client::handleProductDetail()
{
...
{
    if (!isLoggedIn || currentUserType != "customer")
    {
        std::cout << "请先以消费者身份登录。" << std::endl;
        return;
    }

    std::string request = "GET_CART|" + currentUsername;
    std::string response;
        ...
        if (status == "CART")
        {
            ...
            do
            {
                ...
                case 1: // 修改商品数量
                {
                    ...
                    if (newQty == 0)
                    {
                        updateRequest = "REMOVE_FROM_CART|" + currentUsername + "|" + nameToUpdate + "|" + sellerName;
                    }
                    else
                    {
                        updateRequest = "UPDATE_CART_ITEM|" + currentUsername + "|" + nameToUpdate + "|" + sellerName + "|" + std::to_string(newQty);
                    }
                ...
                case 2: // 移除商品
                {
                    ...
                    // 发送移除请求
                    std::string removeRequest = "REMOVE_FROM_CART|" + currentUsername + "|" + nameToRemove + "|" + sellerName;
...
void Client::handleAddToCart()
{
    ...
    std::string request = "ADD_TO_CART|" + currentUsername + "|" + productName + "|" + sellerName + "|" + std::to_string(quantity);
    std::string response;
...
void Client::handleCheckout()
{
    ...
    std::string request = "CHECKOUT|" + currentUsername;
    std::string response;
...
void Client::handleSellerInfo()
{
    if (!isLoggedIn || currentUserType != "seller")
    {
...
void Client::handleManageProducts()
{
    if (!isLoggedIn || currentUserType != "seller")
    {
        std::cout << "请先以商家身份登录。" << std::endl;
    ...
    while (true)
    {
        ...
        {
            // 查看我的商品
            std::string request = "GET_SELLER_PRODUCTS|" + currentUsername;
            std::string response;
        ...
        case 2:
        {
            ...
            { // 书籍
                ...
                request = "ADD_BOOK|" + currentUsername + "|" + name + "|" + description + "|" +
            ...
            { // 服装
                ...
                request = "ADD_CLOTHING|" + currentUsername + "|" + name + "|" + description + "|" +
            ...
            { // 食品
                ...
                request = "ADD_FOOD|" + currentUsername + "|" + name + "|" + description + "|" +
            ...
            { // 其他
                ...
                request = "ADD_GENERIC|" + currentUsername + "|" + name + "|" + description + "|" +
        ...
        case 3:
        {
            ...
            std::string request = "UPDATE_PRODUCT_PRICE|" + currentUsername + "|" + productName + "|" + std::to_string(newPrice);
            std::string response;
        ...
        case 4:
        {
            ...
            std::string request = "UPDATE_PRODUCT_QUANTITY|" + currentUsername + "|" + productName + "|" + std::to_string(newQuantity);
            std::string response;
        ...
        case 5:
        {
            ...
            if (discountType == 1)
            {
                ...
                std::string request = "SET_PRODUCT_DISCOUNT|" + currentUsername + "|" + productName + "|" + std::to_string(discount / 100);
                std::string response;
            ...
            else if (discountType == 2)
            {
                ...
                std::string request = "SET_CATEGORY_DISCOUNT|" + currentUsername + "|" + category + "|" + std::to_string(discount);
                std::string response;
...
void Client::handleViewUserInfo()
{
    if (!isLoggedIn)
    {
        std::cout << "请先登录。" << std::endl;
...
void Client::handleCheckBalance()
{
    if (!isLoggedIn)
    {
        std::cout << "请先登录。" << std::endl;
...
void Client::handleEnterStore()
{
    int choice = -1;

    do
    {
        std::cout << "\n--- 商城 (用户: " << currentUsername << ") ---\n";
        std::cout << "1. 显示所有商品\n";
        std::cout << "2. 按名称搜索商品\n";
        std::cout << "3. 直接购买商品\n";
        std::cout << "4. 加入购物车\n";
...
void Client::handleSearchProducts()
{
    if (!isLoggedIn)
    {
        std::cout << "请先登录。" << std::endl;
...
void Client::handleDirectPurchase()
{
    if (!isLoggedIn || currentUserType != "customer")
    {
