...
int Customer_page::pagestore(User *currentUser, std::vector<User *> &users, Store &store)
{
    ...
    do
    {
        ...
        case 3: // 直接购买商品逻辑 - 现在通过订单系统处理
        {
            ...
            // 创建订单对象
            Order currentOrder(customer->getUsername());
            currentOrder.setStatus("PENDING_VALIDATION");

            // 添加商品到订单
            if (!currentOrder.addItemFromProduct(*product_direct, quantity_direct))
            {
                cout << "添加商品到订单失败！" << endl;
                break;
            }
            ...
            cout << "您的当前余额: ¥" << std::fixed << std::setprecision(2) << customer->checkBalance() << endl;
            cout << "支付后余额将为: ¥" << (customer->checkBalance() - currentOrder.getTotalAmount()) << endl;
            cout << "--------------------" << endl;
            ...
            // 提交订单到处理队列
            cout << "正在处理订单，请稍候..." << endl;
            extern OrderManager g_orderManager;
            auto submittedOrder = g_orderManager.submitOrderRequest(currentOrder);
            cout << "订单已成功提交到处理队列，订单ID: " << submittedOrder->getOrderId() << endl;

            cout << "正在等待订单处理，请稍候..." << endl;

            // 等待订单处理完成
            int waitCounter = 0;
            const int MAX_WAIT_CYCLES = 100; // 最多等待10秒

            while (!submittedOrder->getProcessed() && waitCounter < MAX_WAIT_CYCLES)
            {
                // 短暂休眠，避免过度消耗CPU
                ...
                if (waitCounter % 10 == 0)
                {
                    cout << "已等待 " << (waitCounter / 10) << " 秒..." << endl;
                }
            }
            ...
            {
                cout << "等待订单处理超时！订单可能仍在处理中。" << endl;
                cout << "您可以稍后查询订单状态。" << endl;
            }
            else
            {
                ...
                {
                    cout << "购买成功！已购买 " << quantity_direct << " 件 \"" << productName_direct << "\"" << endl;
                }
                else if (submittedOrder->getStatus().find("FAILED") != std::string::npos)
                {
                    cout << "订单处理失败! 原因: ";
                    if (submittedOrder->getStatus() == "FAILED_PAYMENT_ERROR")
                        cout << "支付处理时出错" << endl;
                    else if (submittedOrder->getStatus() == "FAILED_INSUFFICIENT_FUNDS")
                        cout << "账户余额不足" << endl;
                    else if (submittedOrder->getStatus() == "FAILED_PRODUCT_NOT_FOUND")
                        cout << "商品不存在或已下架" << endl;
                    else if (submittedOrder->getStatus() == "FAILED_INSUFFICIENT_STOCK")
                        cout << "商品库存不足" << endl;
                    else
                        cout << "未知原因，请联系客服" << endl;
                }
            }
