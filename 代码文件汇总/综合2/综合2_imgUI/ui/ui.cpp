...
void UI::renderStoreCustomer()
{
    ...
    if (currentUser && userType == 1)
    {
        ...
        if (ImGui::Button("直接购买", ImVec2(100, 30)))
        {
            ...
            else
            {
                ...
                else if (currentUser->checkBalance() < product->getPrice() * buyQuantity)
                {
                ...
                {
                    // 设置确认对话框信息并显示
                    confirmTotalAmount = product->getPrice() * buyQuantity;
                    showDirectPurchaseConfirm = true;
                }
            }
        }
    ...
    // 直接购买确认对话框
    if (showDirectPurchaseConfirm)
    {
        // 设置对话框居中显示
        ImVec2 center = ImVec2(width * 0.5f, height * 0.5f);
        ImGui::SetNextWindowPos(center, ImGuiCond_Always, ImVec2(0.5f, 0.5f));

        if (ImGui::BeginPopupModal("确认直接购买", NULL, ImGuiWindowFlags_AlwaysAutoResize))
        {
            Product *product = store->findProductByName(buyProductName);
            if (product)
            {
                ImGui::Text("订单详情:");
                ImGui::Separator();
                ImGui::Text("商品名称: %s", buyProductName);
                ImGui::Text("购买数量: %d", buyQuantity);
                ImGui::Text("单价: CNY %.2f", product->getPrice());
                ImGui::Text("总金额: CNY %.2f", confirmTotalAmount);
                ImGui::Separator();
                ImGui::Text("您的余额: CNY %.2f", currentUser->checkBalance());
                ImGui::Text("支付后余额: CNY %.2f", currentUser->checkBalance() - confirmTotalAmount);
                ImGui::Separator();

                if (ImGui::Button("确认购买", ImVec2(120, 30)))
                {
                    // 执行购买逻辑
                    Order order(currentUser->getUsername());
                    order.addItemFromProduct(*product, buyQuantity);
                    ...
                    showDirectPurchaseConfirm = false;
                    ImGui::CloseCurrentPopup();
                }
...
void UI::renderCartWindow()
{
    ...
    if (ImGui::Begin("购物车", &showCartWindow, ImGuiWindowFlags_NoCollapse))
    {
        ...
        else
        {
            ...
            if (ImGui::BeginTable("CartTable", 6, ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg))
            {
                ...
                // 显示总计
                ImGui::Separator();
                ImGui::Text("总计: CNY %.2f", totalAmount); // 结算按钮
                if (ImGui::Button("结算", ImVec2(120, 30)))
                {
                    if (totalAmount > 0)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                // 设置确认对话框信息并显示
                                confirmTotalAmount = totalAmount;
                                showCartCheckoutConfirm = true;
                            }
                        }
                    }
                }
    ...
    // 购物车结账确认对话框
    if (showCartCheckoutConfirm && customer)
    {
        // 设置对话框居中显示
        ImVec2 center = ImVec2(width * 0.5f, height * 0.5f);
        ImGui::SetNextWindowPos(center, ImGuiCond_Always, ImVec2(0.5f, 0.5f));

        if (ImGui::BeginPopupModal("确认购物车结账", NULL, ImGuiWindowFlags_AlwaysAutoResize))
        {
            ImGui::Text("订单详情:");
            ImGui::Separator();

            // 显示购物车商品明细
            if (ImGui::BeginTable("CheckoutTable", 4, ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg))
            {
                ImGui::TableSetupColumn("商品名称", ImGuiTableColumnFlags_WidthFixed, 150);
                ImGui::TableSetupColumn("数量", ImGuiTableColumnFlags_WidthFixed, 60);
                ImGui::TableSetupColumn("单价", ImGuiTableColumnFlags_WidthFixed, 80);
                ImGui::TableSetupColumn("小计", ImGuiTableColumnFlags_WidthFixed, 80);
                ImGui::TableHeadersRow();
                ...
                for (const auto &item : customer->shoppingCartItems)
                {
                    ...
                    ImGui::TableSetColumnIndex(3);
                    ImGui::Text("CNY %.2f", item.priceAtAddition * item.quantity);
                }
            ...
            ImGui::Separator();
            ImGui::Text("总金额: CNY %.2f", confirmTotalAmount);
            ImGui::Text("您的余额: CNY %.2f", customer->checkBalance());
            ImGui::Text("支付后余额: CNY %.2f", customer->checkBalance() - confirmTotalAmount);
            ImGui::Separator();

            if (ImGui::Button("确认结账", ImVec2(120, 30)))
            {
                // 执行结账逻辑
    ...
    if (ImGui::Begin("我的订单", &showOrderWindow, ImGuiWindowFlags_NoCollapse))
    {
        ...
        if (showPending && !pendingOrders.empty())
        {
            if (ImGui::CollapsingHeader("待处理订单", ImGuiTreeNodeFlags_DefaultOpen))
            {
                ...
                {
                    std::string displayName = "订单 " + pendingOrders[i]->getOrderId().substr(8) + "...";
