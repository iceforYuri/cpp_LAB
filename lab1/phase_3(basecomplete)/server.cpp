...
// 文件路径常量，这些现在是在 globals.cpp 中定义的
extern const string USER_FILE;
extern const string STORE_FILE;
extern const string CART_FILE;
extern const string ORDER_DIR;

// 订单管理器对象，这也在 globals.cpp 中定义
...
void handleClient(SOCKET clientSocket, vector<User *> &users, Store &store, OrderManager &orderManager)
{
    ...
    while (clientConnected && g_serverRunning)
    {
        ...
        else if (command == "GET_PRODUCTS")
        {
            ...
            response = oss.str();
            std::cout << "发送商品列表: " << response << std::endl;
        }
