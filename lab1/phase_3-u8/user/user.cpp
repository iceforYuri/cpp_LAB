...
const std::string CART_DIR = "./server_data/carts"; // 购物车文件存储目录

// 在网络服务端版本中注释掉这些全局变量声明，它们只在UI版本中使用
...
void Customer::viewCart(Store &store, std::vector<User *> &allUsers)
{
    ...
    do
    {
        ...
        case 3: // Generate Order and Checkout
        {
            ...
            cout << "\n正在处理订单，请稍候..." << endl;
            currentOrder.setStatus("PROCESSING"); // 在网络服务端版本中，这部分逻辑由服务端处理，客户端不直接处理订单
            // 这里只是一个占位符，实际的订单处理在服务端的 NetworkServer::handleOrderCreate 中
            cout << "订单信息已准备完成，等待提交到服务端处理..." << endl; // 在网络版本中，订单处理逻辑在服务端完成
            // 这里只显示订单准备完成的消息
            cout << "订单信息准备完成，可以提交到服务端。" << endl;
...
User *User::registerUser(std::vector<User *> &users)
{
    ...
    { // Customer
        // 在服务端版本中使用默认的购物车目录
        const std::string DEFAULT_CART_DIR = CART_DIR;
        newUser = new Customer(newUsername, newPassword, DEFAULT_CART_DIR, initialBalance);
    }
    else
    { // Seller
    ...
    while (getline(file, line))
    {
            ...
            (ss >> bal))
        {
            if (type == "customer")
            {
                // 在服务端版本中使用默认的购物车目录
                const std::string DEFAULT_CART_DIR = CART_DIR;
                loadedUsers.push_back(new Customer(uname, pwd, DEFAULT_CART_DIR, bal));
            }
            else if (type == "seller")
            {
