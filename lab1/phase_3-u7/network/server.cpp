...
void NetworkServer::handleProductAdd(std::shared_ptr<ClientSession> session, const Protocol::Message &message)
{
    ...
    Protocol::ProductData productData = Protocol::ProductData::deserialize(productDataStr); // 根据商品类型创建具体的商品对象，而不是直接实例化抽象Product类
    bool success = false;
    if (productData.type == "Book")
    {
        // 对于图书，使用attributes中的作者和ISBN
        std::string author = productData.attributes.count("author") ? productData.attributes.at("author") : "Unknown Author";
        std::string isbn = productData.attributes.count("isbn") ? productData.attributes.at("isbn") : "N/A";
    ...
    {
        // 对于服装，使用attributes中的尺码和颜色
        std::string size = productData.attributes.count("size") ? productData.attributes.at("size") : "M";
        std::string color = productData.attributes.count("color") ? productData.attributes.at("color") : "Default";
    ...
    {
        // 对于食品，使用attributes中的过期日期
        std::string expirationDate = productData.attributes.count("expirationDate") ? productData.attributes.at("expirationDate") : "2025-12-31";
    ...
    {
        // 默认创建通用商品，使用attributes中的categoryTag
        std::string categoryTag = productData.attributes.count("categoryTag") ? productData.attributes.at("categoryTag") : productData.type;
