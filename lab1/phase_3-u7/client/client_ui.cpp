...
#include <sstream>
#include <iomanip>
#include <cmath> // 为了 std::min, std::max
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (networkClient->lockInventory(targetProduct->id, buyQuantity))
                    {
                        ...
                        setStatus("商品库存已锁定，请确认购买");
                    }
                    else
                    {
                        setError("库存锁定失败！商品可能库存不足或已售完");
                    }
    ...
    if (ImGui::BeginTabItem("添加商品"))
    {
        ...
        if (ImGui::Button("添加商品", ImVec2(120, 30)))
        {
            ...
            else
            {
                ...
                newProduct.originalPrice = newProductPrice; // 设置原价与当前价格相同
                newProduct.quantity = newProductQuantity;
                newProduct.discountRate = 0.0;                    // 新产品默认无折扣
                newProduct.sellerUsername = currentUser.username; // 根据类型设置type和属性
                if (newProductType == 0)
                { // 图书
...
void ClientUI::purchaseProduct(const std::string &productName, int quantity)
{
    ...
    for (const auto &product : allProducts)
    {
        ...
        {
            // 首先尝试锁定库存
            if (!networkClient->lockInventory(product.id, quantity))
            {
                setError("库存锁定失败！商品 \"%s\" 可能库存不足或已售完", productName.c_str());
                return;
            }
            ...
            {
                // 订单创建失败，解锁库存
                if (!networkClient->unlockInventory(product.id, quantity))
                {
...
void ClientUI::renderDirectPurchaseConfirmDialog()
{
    ...
    if (ImGui::BeginPopupModal("直接购买确认", nullptr, ImGuiWindowFlags_AlwaysAutoResize))
    {
        ...
        if (ImGui::Button("确认购买", ImVec2(90, 0)) && canPurchase)
        {
            // 创建直接订单（此时库存已锁定）
            std::string orderId;
            if (networkClient->createDirectOrder(productToPurchase.id, quantityToPurchase, orderId))
            {
                setStatus("直接购买成功！订单ID: %s", orderId.c_str());
                refreshProducts();
            ...
            else
            {
                ...
                {
                    setError("直接购买失败！可能是余额不足，库存已释放");
                }
                else
                {
                    setError("警告：直接购买失败且库存解锁失败！请联系客服处理");
                }
            }
        ...
        if (ImGui::Button("取消", ImVec2(80, 0)))
        {
            ...
            if (!networkClient->unlockInventory(productToPurchase.id, quantityToPurchase))
            {
                setError("警告：库存解锁失败！请联系客服处理");
            }
            else
            {
                setStatus("已取消直接购买，库存已释放");
            }
