...
bool UI::init()
{
    ...
    glfwSetWindowSizeCallback(window, [](GLFWwindow *w, int width, int height)
                              {
        UI* ui = static_cast<UI*>(glfwGetWindowUserPointer(w));
        if (ui) {
            ui->width = width;
            ui->height = height;
            ui->windowSizeChanged = true;
        } });
...
void UI::renderMainMenu()
{
    ...
    ImGui::Spacing();
    ImGui::Spacing();
    float buttonWidth = std::min(300.0f, width * 0.3f);
    float buttonHeight = std::min(50.0f, height * 0.08f);

    ImGui::SetCursorPosX((width - buttonWidth) / 2);
    if (ImGui::Button("注册新用户", ImVec2(buttonWidth, buttonHeight)))
    {
    ...
    ImGui::Spacing();
    ImGui::SetCursorPosX((width - buttonWidth) / 2);
    if (ImGui::Button("用户登录", ImVec2(buttonWidth, buttonHeight)))
    {
    ...
    ImGui::Spacing();
    ImGui::SetCursorPosX((width - buttonWidth) / 2);
    if (ImGui::Button("进入商城 (游客)", ImVec2(buttonWidth, buttonHeight)))
    {
        currentPage = 2; // 切换到商城
    }
    ...
    ImGui::Spacing();
    ImGui::SetCursorPosX((width - buttonWidth) / 2);
    if (ImGui::Button("退出程序", ImVec2(buttonWidth, buttonHeight)))
    {
        shouldClose = true;
    }
}
...
void UI::renderUserMenu()
{
    ...
    ImGui::SetWindowFontScale(1.0f);
    ImGui::Separator();                 // 用户信息
    float childHeight = height * 0.75f; // 使高度根据窗口高度变化
    ImGui::BeginChild("UserInfo", ImVec2(width * 0.4f, childHeight), true);
    ...
    // 操作区域
    ImGui::SameLine();
    ImGui::BeginChild("UserActions", ImVec2(0, childHeight), true);
...
void UI::renderStoreCustomer()
{
    ...
    static char prevSearchBuffer[128] = {0};
    bool textChanged = strcmp(prevSearchBuffer, searchBuffer) != 0;

    if (ImGui::InputText("搜索商品", searchBuffer, IM_ARRAYSIZE(searchBuffer), ImGuiInputTextFlags_EnterReturnsTrue) || textChanged)
    {
        // 保存当前搜索内容以便下次对比
        strcpy(prevSearchBuffer, searchBuffer);
    ...
    else if (strlen(searchBuffer) > 0) // 有搜索关键词但没有结果
    {
        ImGui::TextColored(ImVec4(0.7f, 0.3f, 0.3f, 1.0f), "未找到匹配商品");
    }
    else // 没有搜索关键词
    {
...
void UI::renderStoreSeller()
{
    ...
    {
        ImGui::BeginChild("MyProducts", ImVec2(0, height * 0.7f)); // 使用相对高度
    ...
    {
        ImGui::BeginChild("AddProduct", ImVec2(0, height * 0.7f)); // 使用相对高度
    ...
    {
        ImGui::BeginChild("ManageProducts", ImVec2(0, height * 0.7f)); // 使用相对高度
...
void UI::renderProductList(const std::vector<Product *> &products, const std::string &title)
{
    ...
    if (ImGui::BeginTable("ProductTable", 6, ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg | ImGuiTableFlags_Resizable))
    {
        // 设置列，确保列数与BeginTable中定义的一致
        ...
        for (const auto &product : products)
        {
            ...
            ImGui::Text("%s", product->getType().c_str());
