...
{
    Product::display();
    std::cout << "  过期日期: " << expirationDate << std::endl;
}
...
{
    Product::display();
    // std::cout << "  类型: " << generalType << std::endl;
}

void General::save(std::ofstream &ofs) const
{
    Product::save(ofs);
    ofs << "," << generalType;
}
...
bool Store::loadSellerProducts(const string &sellerUsername)
{
    ...
    while (getline(file, line))
    {
        ...
        try
        {
            ...
            else if (seglist.size() >= 8)
            {
                // General类型或其他未知类型，将其视为General
                newProduct = new General(name, desc, price, qty, seglist[7], seller);
            }
...
bool Store::createGeneral(User *currentUser, const string &name, const string &desc,
                          double price, int qty, const string &generalType)
{
    if (!currentUser)
    {
        cerr << "错误: 用户未登录" << endl;
    ...
    try
    {
        General *newGeneral = new General(name, desc, price, qty, generalType, sellerUsername);
        allProducts.push_back(newGeneral);

        // 更新商家商品映射
        sellerProducts[sellerUsername].push_back(newGeneral);
