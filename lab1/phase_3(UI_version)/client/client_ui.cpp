...
bool ClientUI::init()
{
    ...
    // 创建窗口
    window = glfwCreateWindow(width, height, "电子商城系统 - 客户端", nullptr, nullptr);
    if (!window)
    {
...
{ // 增加高度以容纳购物车和订单按钮
    ...
    // 实时搜索：移除 ImGuiInputTextFlags_EnterReturnsTrue 标志，让输入框在内容改变时立即响应
    bool searchTextChanged = ImGui::InputText("搜索商品", searchBuffer, IM_ARRAYSIZE(searchBuffer));
    ...
    if (isLoggedIn && userType == Protocol::UserType::CUSTOMER)
    {
        ...
        if (ImGui::Button("查看购物车", ImVec2(150, 30)))
        { // 增加按钮高度使其更明显
            showCartWindow = true;
            refreshCart();
        }
        ...
        if (ImGui::Button("查看订单", ImVec2(150, 30)))
        { // 和购物车按钮保持一致的大小
        ...
        if (ImGui::Button("直接购买", ImVec2(100, 30)))
        {
            ...
            else
            {
                ...
                else
                {
                    // 直接购买逻辑 - 使用新的直接购买方法
                    std::string orderId;
                    if (networkClient->createDirectOrder(targetProduct->id, buyQuantity, orderId))
                    {
    ...
    if (ImGui::BeginTabItem("添加商品"))
    {
        ...
        if (ImGui::Button("添加商品", ImVec2(120, 30)))
        {
            ...
            else
            {
                ...
                {                                      // 其他
                    newProduct.type = newProductAttr1; // 使用自定义类型名
                }
...
void ClientUI::renderCartWindow()
{
    ...
    ImGui::SetNextWindowPos(ImVec2(width / 2 - 350, height / 2 - 250));
    ...
    if (ImGui::Begin("购物车", &showCartWindow, ImGuiWindowFlags_NoCollapse))
    {
        ...
        else
        {
            if (ImGui::BeginTable("CartTable", 6, ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg | ImGuiTableFlags_ScrollY))
            {
                ...
                ImGui::TableSetupColumn("数量", ImGuiTableColumnFlags_WidthFixed, 100.0f); // 增加数量列宽度从80到100
                ...
                for (size_t i = 0; i < cartItems.size(); ++i)
                {
                    ...
                    ImGui::SetNextItemWidth(75.0f);                 // 增加宽度从60到75
...
void ClientUI::refreshProducts()
{
    ...
    allProducts.clear();
    networkClient->getAllProducts(allProducts);
}
...
void ClientUI::purchaseProduct(const std::string &productName, int quantity)
{
    ...
    for (const auto &product : allProducts)
    {
        if (product.name == productName)
        {
            ...
            {
                setStatus("直接购买成功！订单ID: %s", orderId.c_str());
                refreshProducts();
            }
            else
            {
                setError("直接购买失败！可能是余额不足或库存不足");
            }
            return;
        }
    }
...
void ClientUI::renderCartCheckoutConfirmDialog()
{
    ...
    // 自动打开弹窗
    if (!ImGui::IsPopupOpen("购物车结算确认"))
    {
        ImGui::OpenPopup("购物车结算确认");
    }
    ...
    if (ImGui::BeginPopupModal("购物车结算确认", NULL, ImGuiWindowFlags_AlwaysAutoResize))
    {
        ImGui::Text("购物车结算确认");
        ImGui::Separator();
        ...
        else
        {
            ...
            if (ImGui::BeginTable("CheckoutSummary", 4, ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg))
            {
                ImGui::TableSetupColumn("商品名称");
                ImGui::TableSetupColumn("单价");
                ImGui::TableSetupColumn("数量");
                ...
                for (const auto &item : cartItems)
                {
                    ...
                    ImGui::TableSetColumnIndex(1);
                    // 使用当前的折扣后价格
                    double currentPrice = item.priceAtAddition;
                    for (const auto &product : allProducts)
                    {
                        if (product.id == item.productId)
                        {
            ...
            ImGui::Text("订单总额: CNY%.2f", totalAmount);
            ...
            if (currentUser.balance < totalAmount)
            {
                ImGui::TextColored(ImVec4(1.0f, 0.4f, 0.4f, 1.0f), "余额不足！请先充值。");
                ImGui::Text("还需充值: CNY%.2f", totalAmount - currentUser.balance);
            }
            else
            {
                ImGui::TextColored(ImVec4(0.4f, 1.0f, 0.4f, 1.0f), "余额充足");
                ImGui::Text("结算后余额: CNY%.2f", currentUser.balance - totalAmount);
            }
            ...
            {
                // 余额充足时显示确认按钮
                if (ImGui::Button("确认结算", ImVec2(120, 30)))
                {
                    // 执行结算操作
                    checkout();
                    showCartCheckoutConfirmDialog = false;
            ...
            {
                // 余额不足时显示充值提示按钮
                if (ImGui::Button("去充值", ImVec2(120, 30)))
                {
                    showCartCheckoutConfirmDialog = false;
                    currentPage = 1; // USER_MENU
                    ImGui::CloseCurrentPopup();
                }
                ImGui::SameLine();
            }
...
void ClientUI::renderDirectPurchaseConfirmDialog()
{
    ...
    if (ImGui::BeginPopupModal("直接购买确认", NULL, ImGuiWindowFlags_AlwaysAutoResize))
    {
        ImGui::Text("直接购买确认");
        ImGui::Separator();
        ...
        ImGui::Text("商品价格: CNY%.2f", productToPurchase.price);
        ImGui::Text("购买数量: %d", quantityToPurchase);

        double totalCost = productToPurchase.price * quantityToPurchase;
        ImGui::Text("总计金额: CNY%.2f", totalCost);
        ...
        {
            ImGui::TextColored(ImVec4(1.0f, 0.4f, 0.4f, 1.0f), "余额不足！请先充值。");
            ImGui::Text("还需充值: CNY%.2f", totalCost - currentUser.balance);
        }
        else
        {
            ImGui::TextColored(ImVec4(0.4f, 1.0f, 0.4f, 1.0f), "余额充足");
            ImGui::Text("购买后余额: CNY%.2f", currentUser.balance - totalCost);
        }
        ...
        if (currentUser.balance >= totalCost)
        {
            if (ImGui::Button("确认购买", ImVec2(120, 30)))
            {
                // 执行购买操作
...
void ClientUI::renderPurchaseConfirmDialog()
{
    ...
    // 自动打开弹窗
    if (!ImGui::IsPopupOpen("购买确认"))
    {
        ImGui::OpenPopup("购买确认");
    }
    ...
    if (ImGui::BeginPopupModal("购买确认", NULL, ImGuiWindowFlags_AlwaysAutoResize))
    {
        ImGui::Text("确认购买操作");
        ImGui::Separator();

        if (ImGui::Button("确认", ImVec2(120, 30)))
        {
...
void ClientUI::renderPasswordChangeDialog()
{
    ...
    static char confirmPass[128] = {0};

    if (ImGui::BeginPopupModal("修改密码", NULL, ImGuiWindowFlags_AlwaysAutoResize))
    {
        ImGui::Text("请输入密码信息");
        ImGui::Separator();
        ...
        ImGui::SetNextItemWidth(200);
        ImGui::InputText("确认新密码", confirmPass, IM_ARRAYSIZE(confirmPass), ImGuiInputTextFlags_Password);

        ImGui::Separator();

        if (ImGui::Button("确认修改", ImVec2(100, 30)))
        {
            if (strlen(oldPass) == 0 || strlen(newPass) == 0)
            {
                showPasswordChangeDialog = false;
                showPasswordChangeFailDialog = true;
                rechargeMessage = "密码不能为空！";
            }
            else if (strcmp(newPass, confirmPass) != 0)
            {
                showPasswordChangeDialog = false;
                showPasswordChangeFailDialog = true;
                rechargeMessage = "两次输入的新密码不一致！";
            }
            else if (networkClient && networkClient->changePassword(oldPass, newPass))
            {
                showPasswordChangeDialog = false;
            ...
            else
            {
                showPasswordChangeDialog = false;
                showPasswordChangeFailDialog = true;
                rechargeMessage = "密码修改失败，请检查原密码是否正确！";
            }
            ImGui::CloseCurrentPopup();
        }
        ImGui::SameLine();

        if (ImGui::Button("取消", ImVec2(100, 30)))
        {
            showPasswordChangeDialog = false;
            memset(oldPass, 0, sizeof(oldPass));
            memset(newPass, 0, sizeof(newPass));
...
void ClientUI::renderPasswordChangeFailDialog()
{
    ...
    if (ImGui::BeginPopupModal("密码修改失败", NULL, ImGuiWindowFlags_AlwaysAutoResize))
    {
        ImGui::Text("密码修改失败");
        ImGui::Separator();
        ImGui::Text("%s", rechargeMessage.c_str());
...
void ClientUI::renderRechargeDialog()
{
    ...
    if (ImGui::BeginPopupModal("账户充值", NULL, ImGuiWindowFlags_AlwaysAutoResize))
    {
        ImGui::Text("账户余额充值");
        ImGui::Separator();
        ...
        ImGui::SetNextItemWidth(200);
        ImGui::InputFloat("充值金额", &rechargeAmount, 10.0f, 100.0f, "%.2f");

        ImGui::Spacing();
        ImGui::Text("充值后余额: CNY%.2f", currentUser.balance + rechargeAmount);

        ImGui::Separator();

        if (ImGui::Button("确认充值", ImVec2(100, 30)))
        {
            if (rechargeAmount <= 0)
            {
                showRechargeDialog = false;
                showRechargeFailDialog = true;
                rechargeMessage = "充值金额必须大于0！";
            }
            else if (networkClient && networkClient->updateBalance(rechargeAmount))
            {
                currentUser.balance += rechargeAmount;
                showRechargeDialog = false;
                showRechargeSuccessDialog = true;
                rechargeMessage = "充值成功！当前余额: CNY" + std::to_string(currentUser.balance);
                rechargeAmount = 0.0f;
            }
            else
            {
