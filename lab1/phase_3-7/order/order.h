...
public:
    ...
    void calculateTotalAmount();                                   // Recalculates total based on items    // Getters
    ...
    // 移除重复的isProcessed()方法，使用getProcessed()代替

    // 添加一个直接从购物车商品添加到订单的方法
    void addProduct(const std::string &productName, const std::string &sellerUsername, int quantity, double price)
    {
        items.emplace_back(productName, productName, quantity, price, sellerUsername);
